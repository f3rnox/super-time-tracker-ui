...
export function PomodoroView({ known_tags }: { known_tags: string[] }) {
  ...
  const [timer_state, setTimer_state] = useState<PomodoroTimerState>(
    POMODORO_DEFAULT_STATE,
  );
  const [now_ms, setNow_ms] = useState<number>(() => Date.now());
  const [is_hydrated, setIs_hydrated] = useState(false);
  const [sync_error, setSync_error] = useState<string | null>(null);
  const [has_running_tracker_entry, setHas_running_tracker_entry] =
    useState(false);
  ...
  const notify_pomodoro_event = useCallback(
      ...
      try {
        ...
        if (stored_value !== null) {
          ...
          if (stored_state !== undefined) {
            ...
            setTimer_state(restored_timer_state);
          }
        }
      } catch {
  ...
  const has_any_running_tracker_entry =
    useCallback(async (): Promise<boolean> => {
      ...
      return (
        payload?.runningEntry !== null && payload?.runningEntry !== undefined
      );
    }, []);

  const refresh_running_tracker_entry_status =
    useCallback(async (): Promise<void> => {
      try {
        setHas_running_tracker_entry(await has_any_running_tracker_entry());
      } catch {
  ...
  const start_tracker_entry_for_pomodoro = async (): Promise<void> => {
    ...
    if (has_running_entry) {
      setHas_running_tracker_entry(true);
      return;
    }
  ...
  const stop_active_tracker_entry_for_pomodoro = async (): Promise<void> => {
    ...
    if (response.ok) {
      clear_pomodoro_task_entry_marker();
      setHas_running_tracker_entry(false);
      return;
    }
  ...
  const start_timer = (): void => {
    ...
    if (!is_resuming && timer_state.status !== "running") {
      notify_pomodoro_event(
        ...
        is_break_phase ? "Break timer is running." : "Focus timer is running.",
      );
    }
    ...
    void start_tracker_entry_for_pomodoro().catch((error: unknown) => {
      setSync_error(error instanceof Error ? error.message : String(error));
    });
  };
  ...
  const progress_percent = Math.min(
    ...
    Math.max(
      0,
      ((current_duration_ms - remaining_ms) / current_duration_ms) * 100,
    ),
  );
          ...
          <div className="flex flex-col gap-4">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                ...
                <p className="m-0 mt-1 text-[1.3rem] font-semibold">
                  {phase_label}
                </p>
            ...
            <div className="rounded-md border border-panel-border bg-background px-4 py-5 text-center">
              ...
              <div className="mt-3 flex flex-wrap items-center justify-center gap-2 text-[0.82rem] text-muted">
                <span>
                  {timer_state.completed_work_sessions} focus sessions done
                </span>
                <span>•</span>
                <span>
                  {timer_state.status === "running"
