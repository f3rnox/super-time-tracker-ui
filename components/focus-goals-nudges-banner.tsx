...
import { get_focus_nudge_alert_class } from "@/lib/get_focus_nudge_alert_class";
import { get_pomodoro_phase_label } from "@/lib/get_pomodoro_phase_label";
...
export function FocusGoalsNudgesBanner({
  ...
  useEffect(() => {
    const read_activePomodoro = (): ActivePomodoroDetails | null => {
      try {
        const raw_value = globalThis.localStorage.getItem(POMODORO_STORAGE_KEY);
    ...
    const initial_refresh_timeout = globalThis.setTimeout(() => {
      setActivePomodoro(read_activePomodoro());
    }, 0);
    ...
    return () => {
      globalThis.clearTimeout(initial_refresh_timeout);
  ...
  useEffect(() => {
    ...
    const refresh_status = async (): Promise<void> => {
      try {
        ...
        query.set(
          "scope",
          resolve_goal_scope(goal_scope, goal_sheet_name, goal_tag_name),
        );
        ...
        const response = await fetch(
          `/api/focus-nudges/status?${query.toString()}`,
        );
        if (!response.ok) {
          return;
        }
  ...
  const content = useMemo<BannerContent | null>(() => {
    ...
    if (resolved_daily === null || resolved_weekly === null) {
      return {
        ...
        detail:
          "No goals configured yet for this scope. Set daily and weekly targets in Goal settings.",
      };
    }
  ...
  return (
    <section className={alert_class} aria-live="polite" aria-atomic="true">
      ...
      <div className="mt-1 flex flex-wrap items-center gap-2">
        ...
        {content.show_check_in_shortcut &&
        on_check_in_shortcut !== undefined ? (
          <button
            type="button"
...
function resolve_target_minutes(
  ...
  if (args.scope === "sheet" && args.sheet_name.length > 0) {
    const entry = parse_focus_goals_by_name(args.per_sheet_json)[
      args.sheet_name
    ];
...
interface FocusProgressCardProps {
  ...
  activePomodoro: ActivePomodoroDetails | null;
}
...
function FocusProgressCard({
  ...
  duration_format,
  activePomodoro,
}: Readonly<FocusProgressCardProps>) {
  const percent =
    target_ms > 0 ? Math.min(100, (tracked_ms / target_ms) * 100) : 0;
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-2 flex items-baseline justify-between gap-3">
            ...
            <p className="m-0 text-[0.78rem] text-muted">
              {format_duration(remaining_ms, duration_format)} left{" "}
              {scope_label}
            </p>
          </div>
          <progress
            className="mt-3 h-1.5 w-full overflow-hidden rounded-full bg-accent-soft [&::-webkit-progress-bar]:rounded-full [&::-webkit-progress-bar]:bg-accent-soft [&::-webkit-progress-value]:rounded-full [&::-webkit-progress-value]:bg-accent [&::-moz-progress-bar]:rounded-full [&::-moz-progress-bar]:bg-accent"
            max={100}
            value={Math.round(percent)}
          />
        </div>
        {activePomodoro === null ? null : (
