...
const PWA_INSTALL_NOTICE_DISMISSED_KEY =
  "super-time-tracker-pwa-install-notice-dismissed";
...
export function PwaInstallNotice() {
  const [deferred_prompt, setDeferred_prompt] =
    useState<BeforeInstallPromptEvent | null>(null);
  const [is_dismissed, setIs_dismissed] = useState(true);
  const [is_installing, setIs_installing] = useState(false);
      ...
      <div className="mt-3 flex gap-2">
        {deferred_prompt !== null ? (
          <button
            ...
            onClick={() => {
              setIs_installing(true);
              void deferred_prompt
                .prompt()
                .then(async () => {
                  ...
                  if (choice.outcome === "accepted") {
                    window.localStorage.setItem(
                      PWA_INSTALL_NOTICE_DISMISSED_KEY,
                      "true",
                    );
                    setIs_dismissed(true);
                  }
