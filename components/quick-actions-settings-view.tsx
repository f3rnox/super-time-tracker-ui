...
export function QuickActionsSettingsView() {
  ...
  const [draft_shortcuts, set_draft_shortcuts] = useState(shortcut_map)
  ...
  useEffect(() => {
    set_draft_shortcuts(shortcut_map)
  }, [shortcut_map])
  ...
  const set_shortcut = (action_id: TrackerShortcutActionId, next_key: string): void => {
  ...
  if (!is_client_ready) {
    return (
      ...
      >
        <p className="m-0 text-[0.85rem] text-muted">Loading quick action settings…</p>
      </SettingsPageLayout>
    )
  }
    ...
    >
      <div className="flex flex-col gap-2">
        <section className="rounded-md border border-panel-border bg-panel p-3.5 shadow-sm">
          <h2 className="m-0 text-[0.95rem] font-semibold">Keyboard shortcuts</h2>
          <p className="m-0 mt-1 text-[0.8rem] leading-snug text-muted">
            Choose one key per action. If a key is reused, only the first matching
            action will trigger.
          </p>
