...
export function SheetSidebar({
  ...
  const [edited_sheet_name, setEdited_sheet_name] = useState("");

  const cancel_add_sheet = (): void => {
    setIs_adding_sheet(false);
    setNew_sheet_name("");
  };

  const handle_create: NonNullable<ComponentProps<"form">["onSubmit"]> = (
    event,
  ) => {
  ...
  const start_rename = (sheet_name: string): void => {
    setEditing_sheet_name(sheet_name);
    setEdited_sheet_name(sheet_name);
  };

  const cancel_rename = (): void => {
    setEditing_sheet_name(null);
    setEdited_sheet_name("");
  };

  use_escape_to_cancel(cancel_rename, editing_sheet_name !== null);
  use_escape_to_cancel(cancel_add_sheet, is_adding_sheet);

  const handle_rename: NonNullable<ComponentProps<"form">["onSubmit"]> = (
    event,
  ) => {
    event.preventDefault();
          ...
          <li key={sheet.name} className="min-w-0">
            {editing_sheet_name === sheet.name ? (
              <form
                ...
                onSubmit={handle_rename}
              >
                <input
                  className={get_input_class_name("compact")}
                  value={edited_sheet_name}
                  onChange={(event) => setEdited_sheet_name(event.target.value)}
                  disabled={is_pending}
                  autoFocus
                />
                ...
                >
                  <span
                    className={`min-w-0 flex-1 truncate font-semibold ${
                      sheet.isActive || sheet.hasActiveEntry
                        ? "text-accent"
                        : ""
                    }`}
                  >
      ...
      {is_adding_sheet ? (
        ...
        >
          <input
            ...
            onChange={(event) => setNew_sheet_name(event.target.value)}
        ...
        <button
          ...
          disabled={is_pending}
          onClick={() => setIs_adding_sheet(true)}
        >
          Add sheet
        </button>
      )}
