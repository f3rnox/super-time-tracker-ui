 ...
 */
export function TrackerDocumentTitle({
  active_entry,
}: Readonly<TrackerDocumentTitleProps>) {
  ...
  const [duration_ms, setDuration_ms] = useState(active_entry?.durationMs ?? 0);
  ...
  useEffect(() => {
    ...
    setDuration_ms(active_entry.durationMs);

    const interval = window.setInterval(() => {
      setDuration_ms(Date.now() - new Date(active_entry.start).getTime());
    }, 1000);
  ...
  useEffect(() => {
    ...
    const label = active_entry.description.trim() || "Tracking";
    const duration = format_duration(
      duration_ms,
      duration_format,
      show_seconds,
    );
