...
export function EntrySearchView({
  initial_filters,
  initial_data,
}: EntrySearchViewProps) {
  const router = useRouter()
  const [filters, set_filters] = useState(initial_filters)
  ...
  useEffect(() => {
    router.replace(build_entry_search_href(debounced_filters), { scroll: false })
    ...
    <>
      ...
      <main className="mx-auto flex w-full max-w-3xl flex-col gap-5 px-5 pb-12 pt-6">
        <header className="flex flex-col gap-2">
          <h1 className="m-0 text-[1.5rem] font-[650] tracking-tight">Search entries</h1>
          <p className="m-0 text-[0.9rem] leading-relaxed text-muted">
            Search across descriptions, @tags, and notes. Filter by sheet, tag, or
            date range.
          </p>
        </header>
        ...
        <label className="flex min-w-0 flex-col gap-1">
          ...
          <input
            ...
            disabled={is_pending}
            onChange={(event) =>
              set_filters((current) => ({ ...current, query: event.target.value }))
            }
          />
        </label>
