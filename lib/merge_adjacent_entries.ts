...
export async function merge_adjacent_entries(
  ...
  const entry_index = sheet.entries.findIndex(
  ...
  const sorted_index = sorted.findIndex(
    (candidate) => candidate.id === entry_id,
  );
  ...
  const earlier_end = earlier.end;
  const later_end = later.end;

  if (earlier_end === null || later_end === null) {
    throw new Error("Cannot merge an entry without an end time");
  }
  ...
  earlier.end = later_end;
  earlier.tags = merged_tags;
  earlier.notes = merged_notes;

  if (
    later.description.trim().length > 0 &&
  ...
  validate_entry_times(earlier.start, later_end);

  const later_index = sheet.entries.findIndex(
    (candidate) => candidate.id === later.id,
  );
