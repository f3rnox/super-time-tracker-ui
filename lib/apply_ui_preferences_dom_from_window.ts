...
import { THEME_MODE_STORAGE_KEY, type ThemeMode } from '@/lib/types/ui_preferences'
...
export function apply_ui_preferences_dom_from_window(): void {
  ...
  try {
    const compact_lists = window.localStorage.getItem(COMPACT_LISTS_STORAGE_KEY)
  ...
  try {
    ...
    if (
      theme_mode !== null &&
      theme_mode_preference.is_valid(theme_mode)
    ) {
