...
const POMODORO_TASK_MARKER_STORAGE_KEY = 'super-time-tracker-pomodoro-active-entry'
...
const read_marker = (): PomodoroTaskMarker | null => {
  ...
  try {
    const raw_value = window.localStorage.getItem(POMODORO_TASK_MARKER_STORAGE_KEY)
 ...
 */
export function mark_pomodoro_task_entry(entry: Pick<SerializedEntry, 'sheetName' | 'id'>): void {
  if (typeof window === 'undefined') {
    return
  }
  ...
  try {
    ...
    window.localStorage.setItem(POMODORO_TASK_MARKER_STORAGE_KEY, JSON.stringify(marker))
  } catch {
...
export function is_pomodoro_task_entry(entry: SerializedEntry): boolean {
  ...
  return marker !== null && marker.sheetName === entry.sheetName && marker.entryId === entry.id
}
